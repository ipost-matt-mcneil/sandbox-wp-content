...
$top_posts						= ( build_subcategory_query( $has_cross_sell ) ) ? new WP_Query( array_merge(build_subcategory_query( $has_cross_sell ) ) ) : false;
...
<?php if( $top_posts && $top_posts->have_posts() ) : ?>
	<div class="tiles__container <?php echo ( $has_cross_sell !== '' ) ? 'tiles__container--is-subcategory-cross-sell' : '' ?> row">
		<?php while ( $top_posts->have_posts() ) : $top_posts->the_post(); ?>
			<?php if ( ( $subcategory_count === 2 || $subcategory_count === 4 ) && $has_cross_sell ) $last_count = 2; ?>
			<?php for ( $tiles = 0 ;  $tiles < $last_count ; $tiles++ ): ?>
				<div class="tiles__item <?php echo ( $has_cross_sell !== '' ) ? 'tiles__item--is-subcategory-cross-sell' : 'tiles__item--is-subcategory' ?>  <?php echo ( $subcategory_count < 2 && $paged === 1  ) ? 'tiles__item--is-subcategory-feature' : '' ?> large-4 medium-6 small-12 columns">
					<?php if ( $last_count === 2 && $tiles === 0 && $paged === 1  ) : ?>
						<?php get_template_part( 'template-parts/cross-sells/inline' ); ?>
					<?php else : ?>	
						<?php if( ( $subcategory_count === 1 || $subcategory_count === 2 || ( $subcategory_count === 3 && $has_cross_sell == '' ) ) && $paged === 1  ) : ?>
						...
						<div class="card__text">
							<?php if ( ( $subcategory_count === 0 || $subcategory_count === 1 ) && $paged === 1 ) : ?>
								<div class="card__img-overlay card__img-overlay--is-subcategory-feature" style="background-image: url(<?php echo get_the_post_thumbnail_url( $post->ID, 'large' ); ?>);"></div>
								<div class="card__white-overlay"></div>
							<?php endif; ?>
							<?php echo build_breadcrumbs( $post->ID ); ?>
							<a href="<?php echo esc_url( get_permalink() ); ?>" class="card">
								<?php if( $subcategory_count === 0 || $subcategory_count === 1 ) : ?>
